...
import { listTrades, getAvailablePairs } from '@/lib/data/trades'
import Link from 'next/link'
import { Plus, ArrowUpRight, ArrowDownRight, ImageIcon, ExternalLink, Search } from 'lucide-react'
...
export default async function JournalPage({ searchParams }: JournalPageProps) {
    ...
    const [trades, availablePairs] = await Promise.all([
        listTrades(filters),
        getAvailablePairs(),
    ])
    ...
    return (
        <div className="max-w-6xl mx-auto space-y-8 pb-20">
            <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
                ...
                <Link
                    href="/trade"
                    className="flex items-center justify-center gap-2 px-6 py-3.5 bg-blue-600 hover:bg-blue-500 text-white font-bold rounded-2xl transition-all shadow-lg shadow-blue-900/20 active:scale-95"
                >
                    <Plus size={20} />
                    Record New Trade
                </Link>
                        ...
                        >
                            ...
                            {trade.trade_screenshots?.[0] && (
                                <div className="mt-6 aspect-video relative rounded-2xl overflow-hidden bg-neutral-800 border border-neutral-700">
                                    ...
                                    {/* Note: Actual image would require a signed URL which we fetch on demand or pre-fetch */}
                                </div>
                            )}
                        </Link>
                    ))}
