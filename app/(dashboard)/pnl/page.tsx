    ...
    return (
        <div className="max-w-6xl mx-auto space-y-12 pb-20">
            ...
            <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
                <div>
                    <h1 className="text-4xl font-bold tracking-tight text-premium-white">Analytics Terminal</h1>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                {stats.map((stat) => (
                    <div key={stat.label} className="bg-neutral-900 border border-neutral-800 p-8 rounded-[2rem] hover:border-neutral-700 transition-all group">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                ...
                <div className="lg:col-span-2 bg-neutral-900 border border-neutral-800 rounded-[2.5rem] p-8 space-y-8">
                ...
                {/* P&L by Pair */}
                <div className="bg-neutral-900 border border-neutral-800 rounded-[2.5rem] p-8 space-y-8">
                    <h3 className="text-xl font-bold flex items-center gap-3">
                    ...
                    <table className="w-full text-left">
                        <thead>
                            <tr className="bg-neutral-800/30 text-[10px] uppercase tracking-widest text-neutral-500 font-bold">
                                ...
                                <th className="px-8 py-4">Pips</th>
                        ...
                        <tbody className="divide-y divide-neutral-800 text-sm">
                            {recentTrades.map((trade) => (
                                <tr key={trade.id} className="hover:bg-neutral-800/30 transition-colors">
                                    ...
                                    <td className={`px-8 py-6 font-bold ${Number(trade.trade_pnl?.[0]?.pnl_amount) >= 0 ? 'text-green-400' : 'text-red-400'}`}>
                                        {Number(trade.trade_pnl?.[0]?.pnl_amount) >= 0 ? '+' : ''}${trade.trade_pnl?.[0]?.pnl_amount}
                                    </td>
                                    <td className="px-8 py-6 font-mono text-neutral-400">{trade.trade_pnl?.[0]?.pnl_pips}</td>
                                    <td className="px-8 py-6 text-neutral-500">{format(new Date(trade.closed_at), 'MMM dd')}</td>
